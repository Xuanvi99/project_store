...
class Cart {
  ...
  addToCart = async (req, res) => {
    ...
    try {
      ...
      if (!product || product.status === "inactive") {
        return res
          .status(404)
          .json({ errMessage: "Product not found or inactive" });
      }
  ...
  repurchaseProductToCart = async (req, res) => {
    const userId = req.params.userId;
    const { listProductToCart } = req.body;
    let result;
    if (!userId || listProductToCart.length === 0) {
      return res.status(400).json({ errMessage: "Invalid data " });
    }
    try {
      const listCartItem = [];
      for (const productItem of listProductToCart) {
        const cart = await cartModel.findOne({ userId: userId }).exec();
        const { productId, size } = productItem;
        let quantity = Number.parseInt(productItem.quantity);
        ...
        const newCart = await cartModel
          ...
          .lean();
        listCartItem.push(newCart.listProduct[0]);
      }
